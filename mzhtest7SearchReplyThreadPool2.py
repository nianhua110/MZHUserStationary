...
def IterateList(authorId,Fid,FidName,date1):
    ###该函数主要用来遍历页
     ...
     for i in range(pageStart,pageEnd):
         ...
         for item in myItems:    
              GetAuthorArticleAndReplyContent(item[1],authorId,item[2],FidName,i,Fid)
...
if __name__=="__main__":
    ...
    authorId= 167043#raw_input(u'请输入作者ID：');
    registerDate ='2014-7-16'#raw_input(u'请输入开始日期，（作者注册时间），格式:xxxx-xx-xx');
    ...
    plt.xticks(arange(len(AirticleNumeber))+0.5,X,fontproperties=font,rotation='vertical')
    plt.title(u'作者'+str(authorId)+u'在各模块发帖子个数分布图',fontproperties=font)
